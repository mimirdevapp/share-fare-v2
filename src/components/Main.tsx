...
import { Plus, Trash2, Users, ReceiptIndianRupee, Calculator, ListChecks, ReceiptText, Send, ExternalLink, BadgePercent, ScanText, Loader2, X, Edit2, Check, Hash, Minus } from 'lucide-react';
...
export default function Main() {
  ...
  // Toggle a friend's inclusion in the "who ordered this" selection for a new expense.
  // Selecting a friend starts them at quantity 1; deselecting clears their quantity.
  const toggleFriendSelection = (friendId) => {
    setSelectedFriends(prev => {
      ...
      setSelectedQuantities(q => ({ ...q, [friendId]: 1 }));
      return [...prev, friendId];
    });
  };

  const updateSelectedQuantity = (friendId, delta) => {
    setSelectedQuantities(prev => {
      const current = prev[friendId] || 1;
  ...
  const addExpense = () => {
    ...
    const shares = selectedFriends.map(friendId => ({
      ...
      friend_id: friendId,
      quantity: selectedQuantities[friendId] || 1
    }));
  ...
  const startEditingExpense = (expenseId) => {
    const currentShares = expenseShares.filter(es => es.expense_id === expenseId);
    setEditingExpenseId(expenseId);
    setEditingExpenseShares(currentShares.map(es => es.friend_id));
    const quantities = {};
    currentShares.forEach(es => {
      quantities[es.friend_id] = es.quantity || 1;
    });
  ...
  const toggleEditingFriend = (friendId) => {
    setEditingExpenseShares(prev => {
      ...
      setEditingQuantities(q => ({ ...q, [friendId]: 1 }));
      return [...prev, friendId];
    });
  };

  const updateEditingQuantity = (friendId, delta) => {
  ...
  const saveExpenseShares = () => {
    ...
    const newShares = editingExpenseShares.map(friendId => ({
      ...
      friend_id: friendId,
      quantity: editingQuantities[friendId] || 1
    }));
  ...
  const calculateSplit = () => {
    ...
    expenses.forEach(expense => {
      const shares = expenseShares.filter(es => es.expense_id === expense.id);
      const totalQuantity = shares.reduce((sum, s) => sum + (s.quantity || 1), 0);

      if (shares.length > 0 && totalQuantity > 0) {
        shares.forEach(share => {
          if (perPerson[share.friend_id]) {
            const qty = share.quantity || 1;
            const friendCost = expense.cost * (qty / totalQuantity);
            perPerson[share.friend_id].amount += friendCost;

            let label = expense.dish_name;
            if (shares.length > 1) {
              label += qty > 1 ? ` (×${qty})` : ' (shared)';
            }
            perPerson[share.friend_id].items.push(label);
          }
        });
  ...
  // Small reusable +/- quantity stepper used in both "add expense" and "edit expense" flows.
  const QuantityStepper = ({ value, onDecrease, onIncrease }) => (
    <div className="flex items-center gap-1 bg-white/20 rounded-md px-1 py-0.5" onClick={(e) => e.stopPropagation()}>
      <button
        type="button"
        onClick={onDecrease}
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4 mb-4">
                  ...
                  {friends.length > 0 && (
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-2">
                        Who ordered this? {selectedFriends.length > 1 && (
                          <span className="text-slate-400 font-normal">(set quantity per person if it's split unevenly)</span>
                        )}
                      </label>
                        ...
                        {friends.map((friend, index) => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-between gap-2 ${isSelected
                                ? 'bg-blue-600 text-white'
                                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                                }`}
                            >
                              <span className="flex items-center gap-2">
                                <div className={`w-6 h-6 rounded-full ${isSelected ? 'bg-white/20' : getFriendColor(index)} flex items-center justify-center text-xs font-semibold text-white`}>
                                  {getInitials(friend.name)}
                                </div>
                                {friend.name}
                              </span>
                              {isSelected && (
                                <QuantityStepper
                                  value={selectedQuantities[friend.id] || 1}
                                  onDecrease={() => updateSelectedQuantity(friend.id, -1)}
                                  onIncrease={() => updateSelectedQuantity(friend.id, 1)}
                                />
                              )}
                            </button>
                          );
                ...
                <div className="space-y-2">
                  {expenses.map(expense => {
                    const sharesForExpense = expenseShares.filter(es => es.expense_id === expense.id);
                    const sharedByFriends = sharesForExpense
                      .map(es => ({ friend: friends.find(f => f.id === es.friend_id), quantity: es.quantity || 1 }))
                      .filter(entry => entry.friend);
                      ...
                      <div key={expense.id} className="bg-slate-50 px-4 py-3 rounded-lg">
                              ...
                              {!isEditing && sharedByFriends.length > 0 && (
                                <>
                                  ...
                                  <div className="flex items-center gap-1">
                                    {sharedByFriends.map(({ friend, quantity }) => {
                                      const friendIndex = friends.findIndex(f => f.id === friend.id);
                                      return (
                                        <div
                                          key={friend.id}
                                          className="relative"
                                          title={`${friend.name}${quantity > 1 ? ` ×${quantity}` : ''}`}
                                        >
                                          <div
                                          ...
                                          {quantity > 1 && (
                                            <span className="absolute -top-1.5 -right-1.5 bg-slate-800 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                                              {quantity}
                                            </span>
                                          )}
                                        </div>
                        ...
                        {isEditing && (
                          <div className="mt-3 pt-3 border-t border-slate-200">
                            <label className="block text-sm font-medium text-slate-700 mb-2">
                              Who shares this expense? {editingExpenseShares.length > 1 && (
                                <span className="text-slate-400 font-normal">(adjust quantity per person if uneven)</span>
                              )}
                            </label>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                              {friends.map((friend, index) => {
                                const isIncluded = editingExpenseShares.includes(friend.id);
                                return (
                                  <button
                                    key={friend.id}
                                    onClick={() => toggleEditingFriend(friend.id)}
                                    className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-between gap-2 ${isIncluded
                                      ? 'bg-blue-600 text-white'
                                      : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200'
                                      }`}
                                  >
                                    <span className="flex items-center gap-2">
                                      <div className={`w-6 h-6 rounded-full ${isIncluded ? 'bg-white/20' : getFriendColor(index)} flex items-center justify-center text-xs font-semibold text-white`}>
                                        {getInitials(friend.name)}
                                      </div>
                                      {friend.name}
                                    </span>
                                    {isIncluded && (
                                      <QuantityStepper
                                        value={editingQuantities[friend.id] || 1}
                                        onDecrease={() => updateEditingQuantity(friend.id, -1)}
                                        onIncrease={() => updateEditingQuantity(friend.id, 1)}
                                      />
                                    )}
                                  </button>
                                );
